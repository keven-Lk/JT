...
public class HelloAspect {
	/**
	 * 切入点表达式的写法:
	 * 1.within("表名.类名") 粗粒度	匹配的类中的全部方法都会执行通知方法
	...
	public void before(JoinPoint joinPoint) {
		...
		System.out.println("获取目标方法的名称"+target);
	}
	/**
	 * 环绕通知配置如下
	 * 1.必须写切入点表达式
...
//	@Around("within(com.jt.controller.HelloController)")
	...
	/**
	 * 3.后置通知 	要求controller中的全部方法都执行后置通知
	 * execution(* com.jt.controller.*.add*(..))一级目录 ..*所有目录
	 */
	 ...
	 * 4.异常通知
	 */
	@AfterThrowing(value = "execution(* com.jt.controller..*.*(..))",
			throwing="able")
